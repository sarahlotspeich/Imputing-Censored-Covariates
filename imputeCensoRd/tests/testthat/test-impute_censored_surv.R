...
test_that("simple errors for bad input", {
  ...
  # time, delta, or surv not characters
  expect_error(impute_censored_surv(at_time = at_time, time = 2, delta = "delta", surv = "base.surv", data = sample.data), "time must be a character")
  expect_error(impute_censored_surv(at_time = at_time, time = "t", delta = 2, surv = "base.surv", data = sample.data), "delta must be a character")
  expect_error(impute_censored_surv(at_time = at_time, time = "t", delta = "delta", surv = 2, data = sample.data), "surv must be a character")
  
  # data not a data frame
  expect_error(impute_censored_surv(at_time = at_time, time = "t", delta = "delta", surv = "base.surv", data = 2), "data must be a data frame")
  
  # data does not contain specified columns
  expect_error(impute_censored_surv(at_time = at_time, time = "time", delta = "delta", surv = "base.surv", data = sample.data), "column with name time")
  expect_error(impute_censored_surv(at_time = at_time, time = "t", delta = "d", surv = "base.surv", data = sample.data), "column with name d")
  expect_error(impute_censored_surv(at_time = at_time, time = "t", delta = "delta", surv = "surv", data = sample.data), "column with name surv")
...
#   
#   sample.data
#   imputed_surv <- impute_censored_surv(at_time = at_time, time = "t", delta = "delta", surv = "base.surv", data = sample.data)
# })
