...
test_that("simple errors for bad input", {
  ...
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = 2, event = "event", data = sample.data, M = 20), "obs must be a character")
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = "t", event = 2, data = sample.data, M = 20), "event must be a character")
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = "t", event = "event", addl_covar = 2, data = sample.data, M = 20), "addl_covar must be a character")
  
  # approx_beyond not an acceptable option
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = "t", event = "event", data = sample.data, approx_beyond = "none", M = 20), "expo, zero, or carryforward")
  
  # data not a data frame or a matrix
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = "t", event = "event", data = 2, M = 20), "data must be a data frame")
  
  # data does not contain specified columns
  expect_error(condl_mean_impute_bootstrap(fit = sample.fit, obs = "obs", event = "event", data = sample.data, M = 20), "column with name obs")
