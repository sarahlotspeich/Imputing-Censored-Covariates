...
# formula.to.test <- c()
PH.to.test <- c(TRUE, FALSE)
N.to.test <- c(500, 1000)
scaleC.to.test <- c(0.35, 2)
...
res <- data.frame()
for (PH in PH.to.test) {
  for (N in N.to.test) {
    for (scaleC in scaleC.to.test) {
...
res %>% group_by(PH, N, scaleC) %>%
  summarize(b0.mean = mean(b0),
            b1.mean = mean(b1),
            b2.mean = mean(b2),
            b0.se = mean(b0_se),
            b1.se = mean(b1_se),
