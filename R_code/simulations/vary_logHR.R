    ...
    for (pZ in pZ.to.test) {
        ...
        for (form in c("JRSS-C", "SMMR", "BIOMJ", "CORRECT")) {
          fit <- atem_cmi_multivar(data = sim_dat, outcome = "y", time = "t", event = "delta", covar = "z", 
                                    approx_last = "expo", est_S0 = "breslow", atem_formula = form, M = 20)#, 
        ...
        write.csv(res, paste0("INCL_vary_logHR", ifelse(logHR < 0, "neg", ""), abs(logHR), "_n", N, "_seed", sim_seed, ".csv"), row.names = F)
      }
    }
  } 
}
