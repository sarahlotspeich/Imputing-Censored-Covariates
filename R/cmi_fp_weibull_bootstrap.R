...
#' @export
cmi_fp_weibull_bootstrap = function(imputation_formula, analysis_formula, W, Delta, Z, data, fit, infinite_integral = TRUE, maxiter = 100, B = 10) {
  # Size of resample
  n = nrow(data)
  ...
  for (b in 1:B) {
    ...
    if (sum(re_data[, Delta]) < n) {
      ...
      re_data_imp = cmi_fp_weibull(W = W, 
                                   Delta = Delta, 
                                   Z = Z, 
                                   data = re_data, 
                                   fit = re_imp_mod, 
