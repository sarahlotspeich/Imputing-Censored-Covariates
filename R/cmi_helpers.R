...
impute_cens_W_cox <- function(data, W, Delta, Z, S, S0, Xmax, trapezoidal_rule, surv_between, surv_beyond, weibull_params) {
  # Save data frame with survival estimates (ordered)
  surv_df <- data[data[, Delta] == 1, ]
...
extend_and_impute_cox <- function(data, W, Delta, Z, S0, Xmax, trapezoidal_rule, surv_between, surv_beyond) {
  # Create an indicator variable for being uncensored
  uncens <- data[, Delta] == 1
  ...
  # Save parameters to tie Weibull curve into Cox estimator
  Xtilde <- max(data[uncens, W]) 
  rho <- min(data[uncens, S0], na.rm = TRUE) 
  ...
  if (rho > 0) {
    ...
    data$surv <- data[, S0] ^ data[, "HR"]
    
    # Calculate imputed values E(X|X>W,Z)
    data <- impute_cens_W_cox(data = extrap$data, 
                              W = W, 
                              Delta = Delta, 
                              Z = Z, 
                              S = "surv", 
    ...
    data <- impute_cens_W_cox(data = data, 
                              W = W, 
                              Delta = Delta, 
                              Z = Z, 
                              S = "surv", 
